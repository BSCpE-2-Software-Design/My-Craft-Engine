...
TEST_CASE("Craft item successfully") {
    ...
    chest.addIngredient(Ingredient("Iron Ore", 2, "pcs"));
    chest.addIngredient(Ingredient("Wood", 1, "pcs"));
...
TEST_CASE("Craft fails with missing ingredients") {
    ...
    CraftEngine::craft(chest, dagger);
    CHECK_FALSE(chest.hasItem("Dagger"));
}
...
TEST_CASE("Craft fails with low tool tier") {
    ...
    Chest chest;
    chest.items.push_back(Item("Magic Necklace"));
    chest.items.push_back(Item("Magic Necklace"));
    chest.tools.push_back(Tool("Forge Hammer", 1));

    Recipe azureBlade("Azure Blade",
        {
    ...
    CHECK_FALSE(chest.hasItem("Azure Blade"));
}
...
TEST_CASE("Craft succeeds with proper tool tier") {
    ...
    CHECK(chest.hasItem("Azure Blade"));
}